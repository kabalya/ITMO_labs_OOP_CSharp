...
public class AdministratorDoTransaction : IUserComands
{
    private readonly ITransactionInfoPrinter printer;

    public AdministratorDoTransaction(ITransactionInfoPrinter printer)
    {
    ...
    public IUserComand Create(ParsedComand parsedComand)
    {
        ...
        return new TransactionUserComand(administratorId, printer).Create(subcommand);
    }
}